<template>
  <div class="stackont-single-support-ticket-container">
    ...
    <columns>
      <column :desktop="8">
        <div class="stackont-single-ticket-content">
          ...
          <div>
            <add-ticket-thread
                :ticket_id="id"
                :cb-push-notification="show_push_notification"
                @added="getItem"
            />
          </div>
      ...
      <column :desktop="4">
        <widget-box title="Extra Info">
          <list-item v-for="(meta, key) in item.metadata" :key="key" :label="meta.label">
            {{ meta.value }}
          </list-item>
        </widget-box>
        ...
        <widget-box v-if="show_sms_widget" title="SMS Messages">
