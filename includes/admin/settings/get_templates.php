...
<div class="table-responsive">
    <table id="tbl_templates" class="table table-striped table-bordered" cellspacing="5" cellpadding="5">
		...
		foreach ( $fields as $field ) {
			...
			?>
            <tr>
                <td id="wpsc_tag_td_<?php echo $field->slug ?>" class="wpsc_tag_td"
                    onclick="wpsc_insert_editor_text('{<?php echo $field->slug ?>}')">{<?php echo $field->slug ?>}
                </td>
                <td><?php echo htmlentities( $label ) ?></td>
                <td>
                    <button id="wpsc_tag_td_btn_<?php echo $field->slug ?>"
                            onclick="copyToClipboard(this,'<?php echo $field->slug ?>')"
                            class="btn btn-sm btn-copy"><?php _e( 'Copy', 'supportcandy' ) ?></button>
                </td>
            </tr>
			<?php
		}
...
<link rel="stylesheet" type="text/css"
      href="<?php echo WPSC_PLUGIN_URL . 'asset/lib/DataTables/datatables.min.css'; ?>"/>
