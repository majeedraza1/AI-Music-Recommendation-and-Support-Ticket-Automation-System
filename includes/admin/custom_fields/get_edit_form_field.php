...
?>
    ...
    <div class="form-group">
        ...
        <select id="wpsc_tf_personal_info" class="form-control" name="wpsc_tf_personal_info">
            <option <?php echo $wpsc_tf_personal_info == '1' ? 'selected="selected"' : '' ?>
                    value="1"><?php _e( 'Yes', 'supportcandy' ); ?></option>
            <option <?php echo $wpsc_tf_personal_info == '0' ? 'selected="selected"' : '' ?>
                    value="0"><?php _e( 'No', 'supportcandy' ); ?></option>
        </select>
    </div>
    <div class="form-group">
        ...
        <select id="wpsc_tf_type" class="form-control" name="wpsc_tf_type">
			<?php foreach ( $field_types as $key => $field ) :
				...
				?>
                <option <?php echo $selected ?> data-options="<?php echo $field['has_options'] ?>"
                                                value="<?php echo $key ?>"><?php echo $field['label'] ?></option>
			<?php endforeach; ?>
        </select>
    </div>
    <div class="form-group" id="wpsc_tf_options_container"
         style="<?php echo ! $wpsc_tf_has_options ? 'display:none;' : ''; ?>">
        ...
        <textarea style="height:80px !important;" id="wpsc_tf_options" class="form-control"
                  name="wpsc_tf_options"><?php echo stripcslashes( implode( '\n', $wpsc_tf_options ) ); ?></textarea>
    </div>
    <div class="form-group">
        ...
        <select id="wpsc_tf_required" class="form-control" name="wpsc_tf_required">
            <option <?php echo $wpsc_tf_required == '1' ? 'selected="selected"' : '' ?>
                    value="1"><?php _e( 'Yes', 'supportcandy' ); ?></option>
            <option <?php echo $wpsc_tf_required == '0' ? 'selected="selected"' : '' ?>
                    value="0"><?php _e( 'No', 'supportcandy' ); ?></option>
        </select>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
                    ...
                    <li class="wpsp_filter_display_element">
                        <div class="flex-container">
                            <div class="wpsp_filter_display_text">
                                ...
                                <input type="hidden" name="wpsp_tf_condition[]"
                                       value="<?php echo htmlentities( $value ) ?>">
                            </div>
                            <div class="wpsp_filter_display_remove" onclick="wpsc_remove_filter(this);"><i
        ...
        <div class="row" style="background-color:#CCD1D1;padding-top:15px;padding-bottom:15px;">
            <div class="col-sm-5">
                <select id="wpsc_tf_vcf" class="form-control" onchange="wpsc_get_conditional_options(this);"
                        name="wpsc_tf_vcf">
            ...
            <div class="col-sm-2">
                <button type="button" class="form-control btn btn-success"
                        onclick="wpsc_add_field_condition();"><?php _e( 'Add', 'supportcandy' ); ?></button>
            </div>
        </div>
    </div>
    <script>
...
?>
    ...
    <button type="button" class="btn wpsc_popup_action"
            onclick="wpsc_set_edit_form_field(<?php echo htmlentities( $field_id ) ?>);"><?php _e( 'Submit', 'supportcandy' ); ?></button>
<?php
$footer = ob_get_clean();
