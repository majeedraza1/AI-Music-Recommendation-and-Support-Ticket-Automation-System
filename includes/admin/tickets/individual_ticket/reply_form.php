...
$allow_reply                            = true;
if ( ( ! $current_user->has_cap( 'wpsc_agent' ) ) && $status_id == $wpsc_close_ticket_status && ! $wpsc_reply_to_close_ticket ) {
	$allow_reply = false;
}
...
if ( $allow_reply ) {
    ...
    <div class="row wpsc_reply_widget">
        <form id="wpsc_frm_tkt_reply" action="index.html" method="post">
            ...
            <input class="form-control" style="margin-top:10px;" type="text" name="reply_bcc" id="reply_bcc"
                   placeholder="<?php _e( 'BCC (Comma separated list)', 'supportcandy' ); ?>"/>
            ...
            <div class="col-sm-6 submit">
				...
				<?php if ( $wpscfunction->has_permission( 'reply_ticket', $ticket_id ) ): ?>
                    <button type="button" id="wpsc_individual_submit_reply_btn"
                            onclick="javascript:wpsc_submit_reply('reply');" class="btn"
				...
				<?php endif; ?>
				<?php if ( $wpscfunction->has_permission( 'add_note', $ticket_id ) ): ?>
                    <button type="button" id="wpsc_individual_add_note_btn"
                            onclick="javascript:wpsc_submit_reply('note');" class="btn"
