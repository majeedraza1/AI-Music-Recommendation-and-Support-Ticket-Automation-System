...
foreach ( $fields as $field ) {
	...
	if ( $wpsc_tf_type == 3 || $wpsc_tf_type == 10 ) {
		...
		if ( $wpscfunction->has_permission( 'change_agentonly_fields', $ticket_id ) && isset( $_POST[ $field->slug ] ) ) {

			if ( $value != $_POST[ $field->slug ] ) {
				$wpscfunction->change_field( $ticket_id, $field->slug, $_POST[ $field->slug ] );
			}
			$description_attachment = isset( $_POST[ $field->slug ] ) ? intval( $_POST[ $field->slug ] ) : array();
			if ( $wpsc_tf_type == 10 ) {
		...
		} else if ( $wpscfunction->has_permission( 'change_agentonly_fields', $ticket_id ) && ! isset( $_POST[ $field->slug ] ) && $value ) {
		...
		} else if ( $wpscfunction->has_permission( 'change_agentonly_fields', $ticket_id ) && $value ) {
			$wpscfunction->delete_ticket_meta( $ticket_id, $field->slug );
			$log_str = sprintf( __( '%1$s removed %2$s', 'supportcandy' ), '<strong>' . $current_user->display_name . '</strong>', '<strong>' . $field->name . '</strong>' );
			$args    = array(
				'ticket_id'   => $ticket_id,
		...
		if ( $wpscfunction->has_permission( 'change_agentonly_fields', $ticket_id ) && isset( $_POST[ $field->slug ] ) && $_POST[ $field->slug ] ) {
