	...
	class WPSC_Ticket_Field_Formatting {
		...
		public function get_field_val( $field ) {
			...
			if ( $wpsc_tf_type == 10 ) {
                            ...
                            <tr>
                                <td>
                                    <input type="hidden" name="<?php echo $field->slug ?>[]"
                                           value="<?php echo htmlentities( $attach['filename'] ) ?>">
				...
				?>
                <div class="wpsp_sidebar_labels"><strong><?php _e( $field_label, 'supportcandy' ) ?>:</strong> <a
                            href="<?php echo $url ?>" target="_blank"><?php _e( 'Click Here', 'supportcandy' ) ?></a>
                </div>
				<?php
			} elseif ( $wpsc_tf_type == 8 ) {
				?>
                <div class="wpsp_sidebar_labels"><strong><?php _e( $field_label, 'supportcandy' ) ?>:</strong> <a
                            href="<?php echo 'mailto:' . $label ?>"><?php echo htmlentities( $label ) ?></a></div>
				<?php
			} elseif ( $wpsc_tf_type == 5 ) {
				?>
                <div class="wpsp_sidebar_labels"><strong><?php _e( $field_label, 'supportcandy' ) ?>:</strong>
                    <br> <?php echo nl2br( stripcslashes( $label ) ); ?></div>
				<?php
			} elseif ( $wpsc_tf_type == 6 ) {
				?>
                <div class="wpsp_sidebar_labels"><strong><?php _e( $field_label, 'supportcandy' ) ?>
                        :</strong> <?php echo $wpscfunction->datetimeToCalenderFormat( $label ); ?></div>
				<?php
				...
				?>
                <div class="wpsp_sidebar_labels"><strong><?php _e( $field_label, 'supportcandy' ) ?>
                        :</strong> <?php echo htmlentities( $meta_values ) ?></div>
				<?php

			} else if ( $wpsc_tf_type == 1 || $wpsc_tf_type == 2 || $wpsc_tf_type == 4 || $wpsc_tf_type == 9 ) {
				?>
                <div class="wpsp_sidebar_labels"><strong><?php _e( $field_label, 'supportcandy' ) ?>
                        :</strong> <?php echo htmlentities( $label ) ?></div>
				<?php
			} else {
