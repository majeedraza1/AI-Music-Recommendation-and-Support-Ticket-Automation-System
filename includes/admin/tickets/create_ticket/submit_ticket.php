...
// Description
$ticket_description = isset( $_POST['ticket_description'] ) ? wp_kses_post( $_POST['ticket_description'] ) : '';
...
if ( is_user_logged_in() || $wpsc_guest_can_upload_files ) {
	...
	if ( $description_attachment ) {
		$args['desc_attachment'] = $description_attachment;
	}
}
...
foreach ( $fields as $field ) {
	...
	switch ( $tf_type ) {
		...
		case '10':
			...
			if ( $arrVal ) {
				$args[ $field->slug ] = $wpscfunction->sanitize_array( $arrVal );
			}
			break;
...
?>
    <div class="col-sm-12" style="margin-top:20px;">
		<?php echo html_entity_decode( $thankyou_html ) ?>
    </div>
<?php
