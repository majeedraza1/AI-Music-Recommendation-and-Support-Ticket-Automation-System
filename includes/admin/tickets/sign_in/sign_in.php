...
<div class="col-sm-6 col-sm-offset-3 col-md-4 col-md-offset-4" style="margin-bottom:20px;">
	...
	if ( $enable_login_settings == '1' ) { ?>
        ...
        <form id="frm_wpsc_sign_in" action="javascript:wpsc_sign_in();" method="post" style="margin-bottom:5px;">
            ...
            <input id="inputEmail" name="username" class="form-control"
                   placeholder="<?php echo __( 'Username or email', 'supportcandy' ) ?>" required="" autofocus=""
            ...
            <input id="inputPassword" name="password" class="form-control"
                   placeholder="<?php echo __( 'Password', 'supportcandy' ) ?>" required="" autocomplete="off"
                   type="password">
            <div class="checkbox">
                <label>
                    <input name="remember" value="remember-me" type="checkbox">
                    <p style="color:<?php echo $general_appearance['wpsc_text_color'] ?> !important;"><?php echo __( 'Remember me', 'supportcandy' ) ?></p>
                </label>
                <div class="pull-right forgot-password">
                    <a href="<?php echo wp_lostpassword_url() ?>"
                       style="color:<?php echo $general_appearance['wpsc_text_color'] ?> !important;"><?php echo __( 'Forgot your password?', 'supportcandy' ) ?></a>
                </div>
            </div>
            ...
            <button id="wpsc_sign_in_btn" class="btn btn-lg btn-block" type="submit"
                    style="<?php echo $signin_button_css ?>"><?php _e( 'Sign In', 'supportcandy' ) ?></button>
		...
		?>
        <input class="btn btn-lg btn-block" type="button" id="wpsc_login_link"
               onclick="window.location.href='<?php echo $login_url; ?>'"
               value="<?php _e( 'Sign In', 'supportcandy' ); ?>"
               style="margin-top:80px; margin-bottom: 5px; <?php echo $signin_button_css ?>"/>
		<?php
	}
	...
	if ( get_option( 'wpsc_user_registration' ) ) {
		...
		if ( $enable_user_registration_settings == '1' ) {
			?>
            <button class="btn btn-lg btn-block" onclick="wpsc_signup_user();" type="button"
			...
			?>
            <a href="<?php echo $registration_url; ?>" class="btn btn-lg btn-block"
               style="background-color:<?php echo $wpsc_appearance_login_form['wpsc_register_now_button_bg_color'] ?> !important; color:<?php echo $wpsc_appearance_login_form['wpsc_register_now_text_color'] ?> !important;border-color:<?php echo $wpsc_appearance_login_form['wpsc_register_now_button_border_color'] ?> !important;"><?php _e( 'Register Now', 'supportcandy' ) ?></a>
			<?php
		}
	...
	if ( get_option( 'wpsc_allow_guest_ticket' ) ):?>
        ...
        <button id="wpsc_login_continue_as_guest" class="btn btn-lg btn-block" onclick="wpsc_get_create_ticket();"
                type="button"
