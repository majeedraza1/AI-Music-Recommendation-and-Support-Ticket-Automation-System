...
<div class="row wpsc_tl_action_bar"
    ...
    <div class="col-sm-12">
        <button type="button" id="wpsc_load_list_new_ticket_btn" onclick="wpsc_get_create_ticket();"
		...
		<?php if ( $current_user->has_cap( 'wpsc_agent' ) ): ?>
            <button type="button" id="wpsc_load_list_agent_setting_btn" onclick="wpsc_get_agent_setting();"
                    class="btn btn-sm wpsc_action_btn" style="<?php echo $action_default_btn_css ?>"><i
                        class="fas fa-user-cog"></i> <?php _e( 'Agent Settings', 'supportcandy' ) ?></button>
		<?php endif; ?>
        <button type="button" class="btn btn-sm visible-xs visible-sm wpsc_action_btn"
                id="wpsc_load_list_show_filters_btn" onclick="toggle_wpsc_sm_filters(this);"
                style="<?php echo $action_default_btn_css ?>"><i
                    class="fa fa-filter"></i> <?php _e( 'Show Filters', 'supportcandy' ) ?></button>
		<?php if ( $wpsc_show_and_hide_filters ) { ?>
            <button type="button" class="btn btn-sm hidden-xs hidden-sm wpsc_action_btn"
                    id="wpsc_load_list_hide_filters_btn" onclick="toggle_wpsc_md_filters(this);"
                    style="<?php echo $action_default_btn_css ?>"><i
                        class="fa fa-filter"></i> <?php _e( 'Hide Filters', 'supportcandy' ) ?></button>
		<?php } else { ?>
            <button type="button" class="btn btn-sm hidden-xs hidden-sm wpsc_action_btn"
                    id="wpsc_load_list_hide_filters_btn" onclick="toggle_wpsc_md_filters(this);"
                    style="<?php echo $action_default_btn_css ?>"><i
		...
		$reset_type = 'all';
		?>
        <button type="button" class="btn btn-sm wpsc_action_btn" id="wpsc_load_list_reset_filters_btn"
                onclick="wpsc_set_default_filter('<?php echo $reset_type ?>');"
                style="<?php echo $action_default_btn_css ?>"><i
                    class="fa fa-retweet"></i> <?php _e( 'Reset Filters', 'supportcandy' ) ?></button>
		<?php if ( $current_user->has_cap( 'wpsc_agent' ) ): ?>
            <button type="button" class="btn btn-sm wpsc_action_btn" id="wpsc_load_list_auto_refresh_btn"
                    onclick="wpsc_set_toggle_auto_refresh();" style="<?php echo $action_default_btn_css ?>"><i
                        class="fas fa-sync-alt"></i> <span
                        id="wpsc_autorefresh_btn_lbl"><?php _e( 'Auto Refresh : Off', 'supportcandy' ) ?></span>
            </button>
		<?php endif; ?>
		<?php do_action( 'wpsc_add_btn_after_default_filter' ); ?>
		<?php if ( $wpscfunction->has_permission( 'assign_agent' ) ): ?>
            <button type="button" class="btn btn-sm wpsc_btn_bulk_action wpsc_action_btn checkbox_depend hidden"
                    id="btn_assign_agents" onclick="wpsc_get_bulk_assign_agent();"
                    style="<?php echo $action_default_btn_css ?>"><i
                        class="fas fa-users"></i> <?php _e( 'Assign Agent', 'supportcandy' ) ?></button>
		<?php endif; ?>
		<?php if ( $wpscfunction->has_permission( 'change_status' ) ): ?>
            <button type="button" class="btn btn-sm wpsc_btn_bulk_action wpsc_action_btn checkbox_depend hidden"
                    id="btn_change_statuses" onclick="wpsc_get_bulk_change_status()"
                    style="<?php echo $action_default_btn_css ?>"><i
                        class="fa fa-arrow-circle-right"></i> <?php _e( 'Change Status', 'supportcandy' ) ?></button>
		<?php endif; ?>
		<?php if ( $wpscfunction->has_permission( 'delete_ticket' ) ): ?>
            <button type="button" class="btn btn-sm wpsc_btn_bulk_action wpsc_action_btn checkbox_depend hidden"
                    id="btn_delete_tickets" onclick="wpsc_get_delete_bulk_ticket()"
                    style="<?php echo $action_default_btn_css ?>"><i
                        class="fa fa-trash"></i> <?php _e( 'Delete Tickets', 'supportcandy' ) ?></button>
            <button type="button" class="btn btn-sm wpsc_btn_bulk_action wpsc_action_btn checkbox_depend hidden"
                    id="btn_restore_tickets" onclick="wpsc_get_restore_bulk_ticket()"
                    style="<?php echo $action_default_btn_css ?>"><i
                        class="fa fa-window-restore"></i> <?php _e( 'Restore Ticket', 'supportcandy' ) ?></button>
            <button type="button" class="btn btn-sm wpsc_btn_bulk_action wpsc_action_btn checkbox_depend hidden"
                    id="btn_delete_permanently_bulk_ticket" onclick="wpsc_get_delete_permanently_bulk_ticket();"
                    style="<?php echo $action_default_btn_css ?>"><i
                        class="fa fa-trash"></i> <?php _e( 'Delete Tickets Permanently', 'supportcandy' ) ?></button>
		<?php endif; ?>
		<?php
		...
		if ( $wpsc_allow_sign_out ) {
			?>
            <button class="btn btn-sm pull-right" type="button" id="wpsc_login_link"
                    onclick="window.location.href='<?php echo wp_logout_url( $support_page_url ) ?>'"
                    style=" <?php echo $logout_btn_css ?>"><i
                        class="fas fa-sign-out-alt"></i> <?php _e( 'Log Out', 'supportcandy' ) ?></button>
		<?php } ?>
    </div>
</div>
...
<div class="row"
    ...
    <div id="wpsc_md_filters"
         class="col-md-2 hidden-xs hidden-sm <?php echo $wpsc_show_and_hide_filters == 0 ? 'hidden' : 'visible' ?> wpsc_sidebar">
        <div class="row"
             style="background-color:<?php echo $wpsc_appearance_ticket_list['wpsc_filter_widgets_bg_color'] ?> !important;color:<?php echo $wpsc_appearance_ticket_list['wpsc_filter_widgets_text_color'] ?> !important;border-color:<?php echo $wpsc_appearance_ticket_list['wpsc_filter_widgets_border_color'] ?> !important;">
			...
			foreach ( $labels as $key => $label ) {
				if ( ( $current_user->has_cap( 'wpsc_agent' ) && $label['visibility'] == 'agent' ) || ( ! $current_user->has_cap( 'wpsc_agent' ) && $label['visibility'] == 'customer' ) || $label['visibility'] == 'both' ) {
					?>
                    <div onclick="wpsc_set_default_filter('<?php echo $key ?>');"
                         class="wpsp_sidebar_labels <?php echo $key ?> <?php echo $filter['label'] == $key ? 'active' : '' ?>">
        ...
        <div class="row"
			...
			foreach ( $saved_filters as $key => $label ) {
                ...
                <div class="wpsp_sidebar_labels">
                    <span onclick="wpsc_set_saved_filter('<?php echo $key ?>');"
                          class="wpsp_sidebar_labels <?php echo $key ?> <?php echo isset( $filter['save_label'] ) && $filter['save_label'] == $label['save_label'] ? 'active' : '' ?>"><?php echo $label['save_label'] ?></span>
                    <button type="button" class="btn btn-sm btn-danger wpsc_sf_del"
                            onclick="wpsc_delete_saved_filter('<?php echo $key ?>');">
			...
			if ( ! $saved_filters ) {
				echo __( 'No filters found!', 'supportcandy' ) . '<br><br>';
			}
			?>
        </div>
    </div>
