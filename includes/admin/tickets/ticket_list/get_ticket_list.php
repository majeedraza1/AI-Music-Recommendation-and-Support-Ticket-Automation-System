...
<form id="frm_additional_filters" action="javascript:wpsc_set_custom_filter();" method="post">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-10 col-md-offset-1 col-lg-8 col-lg-offset-2 support_ticket_search_box"
        ...
        <div class="wpsp_custom_filter_container" style="display:none;">
				...
				foreach ( $fields as $field ) {
					...
					if ( $filter_type == 'string' || $filter_type == 'number' ) {
                        ...
                        <div id="tf_<?php echo $field->slug ?>" class="form-group col-sm-12">
                            ...
                            <input type="text" data-field="<?php echo $field->slug ?>"
                                   class="form-control wpsc_search_autocomplete"
                                   placeholder="<?php _e( 'Search...', 'supportcandy' ) ?>">
                                    ...
                                    <li class="wpsp_filter_display_element">
                                        <div class="flex-container">
                                            <div class="wpsp_filter_display_text">
                                                ...
                                                <input type="hidden" name="custom_filter[<?php echo $field->slug ?>][]"
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="text" autocomplete="off" class="form-control wpsc_date"
                                       name="custom_filter[<?php echo $field->slug ?>][from]"
                                       value="<?php echo isset( $filter['custom_filter'][ $field->slug ]['from'] ) ? $filter['custom_filter'][ $field->slug ]['from'] : ''; ?>"
                                       placeholder="<?php _e( 'From', 'supportcandy' ) ?>">
                            </div>
                            <div class="col-sm-6">
                                <input type="text" autocomplete="off" class="form-control wpsc_date"
                                       name="custom_filter[<?php echo $field->slug ?>][to]"
                                       value="<?php echo isset( $filter['custom_filter'][ $field->slug ]['to'] ) ? $filter['custom_filter'][ $field->slug ]['to'] : ''; ?>"
                                       placeholder="<?php _e( 'To', 'supportcandy' ) ?>">
                            </div>
                        </div>
						<?php
            ...
            <div class="row wpsp_filter_footer"
                 style="background-color:<?php echo $wpsc_appearance_modal_window['wpsc_footer_bg_color'] ?> !important;">
                <div class="col-sm-12">
                    <button type="submit" id="wpsc_load_apply_filter_btn" class="btn wpsp_filter_btn"
                    ...
                    <button type="button" id="wpsc_save_ticket_filter_btn" onclick="wpsc_get_save_ticket_filter();"
                            class="btn wpsp_filter_btn"
                    ...
                    <button type="button" id="wpsc_load_close_filter" onclick="wpsc_close_custom_filter();"
                            class="btn wpsp_filter_btn"
...
$orderby      = sanitize_text_field( $filter['orderby'] );
...
$count_query = $wpscfunction->get_sql_query( 'COUNT(DISTINCT t.id)', $meta_query, $search );
...
<div class="col-sm-6 col-sm-offset-6" style="margin-top:-20px;text-align:right;font-size:12px;padding-right:0;">
	...
	if ( $total_items > $post_per_page ) {
		?>
        <strong><?php echo ( $current_page * $post_per_page - $post_per_page ) + 1; ?></strong>-
        <strong><?php echo htmlentities( $no_of_tickets ); ?></strong> of
        <strong><?php echo htmlentities( $total_items ); ?></strong> <?php ( $total_items > 1 ) ? _e( 'Tickets', 'supportcandy' ) : _e( 'Ticket', 'supportcandy' ); ?>
		<?php
	} else {
		?>
...
<table id="tbl_support_ticket_list" class="table">
	...
	if ( $ticket_list ) {
		foreach ( $ticket_list as $ticket ) {
			?>
            <tr class="wpsc_tl_row_item" data-id="<?php echo $ticket['id'] ?>"
                onclick="if(link)wpsc_get_individual_ticket(this);">
                <td onmouseover="link=false;" onmouseout="link=true;">
                    <input type="checkbox" name="chk_ticket_list_item[]" class="chk_ticket_list_item"
                           onchange="toggle_ticket_list_actions();" value="<?php echo $ticket['id']; ?>"/>
                </td>
				<?php
				foreach ( $ticket_list_items as $list_item ) {
					?>
