...
<div id="<?php echo $id ?>" class="row" style="background-color:#CCD1D1;padding-top:30px;padding-bottom:30px;">
            ...
            <div class="row wpsc_condition_element" style="margin-bottom:20px;">
                ...
                <div class="col-sm-4">
                    ...
                    <select class="wpsc_condition_field form-control" onchange="wpsc_condition_change(this)">
						...
						<?php foreach ( $condition_options as $index => $option ) :
                            ...
                            <option <?php echo $selected ?> data-hasoptions="<?php echo $option['has_options'] ?>"
                                                            value="<?php echo $option['key'] ?>"><?php echo $option['label'] ?></option>
                ...
                <div class="col-sm-3 wpsc_condition_compare_container">
					...
					if ( $condition_options[ $current_condition_index ]['has_options'] ) {
						...
						?>
                        <select class="wpsc_condition_compare form-control">
                            <option <?php echo $condition->compare == 'contain' ? 'selected="selected"' : '' ?>
                                    value="contain"><?php _e( 'Contains', 'supportcandy' ) ?></option>
                            <option <?php echo $condition->compare == 'match' ? 'selected="selected"' : '' ?>
                                    value="match"><?php _e( 'Matches', 'supportcandy' ) ?></option>
                        </select>
						<?php
                ...
                <div class="col-sm-4 wpsc_condition_value_container">
					...
					if ( $condition_options[ $current_condition_index ]['has_options'] ) {
                        ...
                        <select class="wpsc_condition_value form-control">
							...
							foreach ( $options as $option ) :
                                ...
                                <option <?php echo $selected ?>
                                        value="<?php echo str_replace( '"', "&quot;", stripcslashes( $option['value'] ) ) ?>"><?php echo $option['label'] ?></option>
							<?php
						...
						?>
                        <input type="text" class="wpsc_condition_value form-control"
                               placeholder="<?php _e( 'Search...', 'supportcandy' ) ?>"
                               value="<?php echo str_replace( '"', "&quot;", stripcslashes( $condition->cond_val ) ) ?>">
						<?php
                ...
                <div class="col-sm-1">
                    <button type="button" class="btn btn-danger btn-sm" onclick="wpsc_remove_condition(this)"><i
    ...
    <div class="col-sm-12" style="padding-left:30px;">
        <button type="button" class="btn btn-default btn-sm"
                onclick="wpsc_add_new_condition('<?php echo $id ?>')"><?php _e( 'Add New', 'supportcandy' ) ?></button>
    </div>
        ...
        <div class="col-sm-4">
            <select class="wpsc_condition_field form-control" onchange="wpsc_condition_change(this)">
				...
				<?php foreach ( $condition_options as $option ) : ?>
                    <option data-hasoptions="<?php echo $option['has_options'] ?>"
                            value="<?php echo $option['key'] ?>"><?php echo $option['label'] ?></option>
				<?php endforeach; ?>
            </select>
        </div>
...
<div style="display:none;" class="wpsc_cond_val_tf_template">
    <input type="text" class="wpsc_condition_value form-control"
           placeholder="<?php _e( 'Search...', 'supportcandy' ) ?>" value="">
</div>
