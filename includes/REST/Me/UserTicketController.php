...
class UserTicketController extends ApiController {
	...
	public function get_item( $request ) {
		...
		if ( $supportTicket->get_user_unread_threads_count() > 0 ) {
			SupportTicket::update( [
				'id'                        => $id,
				'user_unread_threads_count' => 0,
			] );
		}
	...
	public function create_thread( $request ) {
		...
		if ( $support_ticket->get_prop( 'agent_created' ) != $user->ID ) {
			return $this->respondUnauthorized();
		}
		...
		$thread_count = $support_ticket->get_admin_unread_threads_count();
		SupportTicket::update( [
			'id'                         => $id,
